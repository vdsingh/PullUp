...
class DatabaseManager{
    ...
    private static func getSessionFromID(id: String, school: String, completion: @escaping(Location?) -> ()){
        
//        return nil
        var location: Location? = nil
        let ref = Database.database().reference()
        ref.child(school).child("sessions").child(id).observeSingleEvent(of: .value, with: { snapshot in
          // Get user value
            guard let value = snapshot.value as? [String: Any] else {
                print("ERROR: problem getting value from database when getting sessionID from user")
                return
            }
//            sessionID = value["currentSession"]
            location = Location(latitude: value["latitude"] as! Double,
                                longitude: value["longitude"] as! Double,
                                locationDescription: value["locationDescription"] as! String,
                                locationSubdescription: value["locationSubdescription"] as! String,
                                sessionGoal: value["sessionGoal"] as! String,
                                courseString: value["course"] as! String,
...
//        return location
    ...
    private static func getSessionIDFromUser(user: User, completion: @escaping(String?) -> ()) {
//        let ref = Auth.auth().data
        let school = user.school
        let ref = Database.database().reference()
        
        var sessionID: String? = nil
        
        ref.child(school).child("users").child(user.safeEmail).observeSingleEvent(of: .value, with: { snapshot in
          // Get user value
            guard let value = snapshot.value as? [String: String] else {
                print("ERROR: problem getting value from database when getting sessionID from user")
                return
...
//            return sessionID
            if sessionID == nil{
                print("LOG: SESSION ID IS NIL")
            }else{
                print("LOG: Session ID: \(sessionID!)")
            }
            completion(sessionID ?? nil)
        }) { error in
          print("ERROR: error trying to get sessionID from users \(error)")
        }
//        print("LOG: returned sessionID: \(sessionID ?? "")")
//        return sessionID
    }
    
    static func getSessionFromUser(user: User, completion: @escaping(Location?) -> ()){
        getSessionIDFromUser(user: user, completion: {sessionID in
            if let id = sessionID{
                getSessionFromID(id: id, school: user.school, completion: {session in
                    if session == nil{
                        print("ERROR: session ID was not nil, but session is nil.")
                    }else{
                        print("LOG: retrieved the session: \(session!)")
                    }
                    completion(session ?? nil)
                })
            }else{
                print("LOG: Session ID is nil when trying to retrieve from user.")
            }
        })
    }
}
